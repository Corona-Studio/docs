...
- 包含: 展示路径的选择框、“重新检查Java可用性”按钮; 重新搜索、深度搜索(仅Windows)、手动添加按钮:  使用选择框来选择一个全局使用的JRE以供启动. 每一条项目均为被搜索到的Java, 并且依次显示了「LauncherX对该Java可用性的判断」、「Java版本」、「该Java的完整路径」. 选中的Java将使用主题色高亮显示, 其版本信息将在选择框收起的情况下于下方并排着的2或3个按钮的下方可见.
